...
def add_item():
    ...
    if partNum_text.get() == '' or description_text.get() == '' or customerName_text.get() == '' or \
        ...
        return
    db.insert(partNum_text.get(), description_text.get(), customerName_text.get(), location_text, unitPrice_text.get(),
              quantity_text.get())
    parts_list.delete(0, END)
...
# Parts List (listBox)
parts_list = Listbox(app, height=20, width=100, border=0)
...
# Buttons
add_btn = Button(app, text='Add Part', width=12, activebackground='blue', command=add_item,)
add_btn.grid(row=3, column=0, pady=20)

remove_btn = Button(app, text='Remove Part', width=12, activebackground='red', command=remove_item)
remove_btn.grid(row=3, column=1)

update_btn = Button(app, text='Update Part', width=12, activebackground='blue',command=update_item)
update_btn.grid(row=3, column=2)

clear_btn = Button(app, text='Clear Input', width=12, activebackground='blue',command=clear_text)
